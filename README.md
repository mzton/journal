...
- Leverage-aware for futures/margin trading: every trade has a leverage
  multiplier (1x = unleveraged), and PnL% is calculated as **ROE
  (return on margin)** rather than return on the full notional value —
  see "How leverage affects the numbers" below
- Automatic PnL ($) and risk:reward calculation — both the *planned*
...
## How leverage affects the numbers

Every trade has a `leverage` field (1x = unleveraged). Two things follow
from that, both handled in `src/utils/calculations.ts`:

- **Margin used** = `(entryPrice × quantity) / leverage` — the capital
  actually tied up to open the position. Shown on the trade form preview
  and the trade detail view.
- **PnL%** is **ROE — return on that margin**, not on the full notional
  value: `PnL($) / marginUsed × 100`. This is the number leverage directly
  amplifies — the same dollar move produces a much bigger % swing at 20x
  than at 2x, because less capital was committed to control the same
  position size.

Two things leverage does **not** change: the **dollar PnL** itself
(`(exit - entry) × quantity`, same as unleveraged) and the **R-multiple**
(`PnL($) / riskAmount($)`) — both are plain dollar-for-dollar figures, so
leverage — which only affects how much margin was required — cancels out
of that ratio.
